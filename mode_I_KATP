...
def SNcATPapopNM(dur, gl, mt, Istim, filename, delay_ms=500,stim_duration_ms=1000,stim_amp_pA=50,scale_I_KATP=1,pulse_width_ms=10,stim_freq_hz=20):
    """
    ...
    s_inf_array = np.zeros(Ttime)
    I_KATP_array = np.zeros(Ttime)
    ...
    beta_ldh_ros = 0.25
    Kldh_ros = 10 * sim_mM
    g_KATP_max = 3.75 #nS #FROM PAPER 45 uS/cm2 --> 45 * A_pmu * 1000 nS
    #K_ATP_sens = 0.1  ##
    s_half = 0.0077 ## 7.7 uM from paper
    ...
    for k in range(Ttime):
        ...
        s_inf = 1.0 /(1.0 + (s_half / ADP)** 1.3) ##
        # E_K =  V_tau * np.log (K_o / K_i) ##
        I_KATP = scale_I_KATP *g_KATP_max *s_inf * (V_snc - V_K* V_tau)  ##
        I_K = I_Ksk + I_Kdr + I_Kir + I_KATP
    ...
    # Figure 1
    fig1 = plt.figure(figsize=(19.2, 11))
    sizz = 15
    ...
    plt.subplot(6, 1, 4)
    plt.plot(sec * dt * np.arange(len(ki_array)), ki_array, 'g')
    plt.xlabel('Time (sec)', fontsize=sizz, fontweight='bold')
    plt.ylabel('K_i (mM)', fontweight='bold')
    plt.title('Intracellular K+', fontsize=sizz, fontweight='bold')

    plt.subplot(6, 1, 5)
    plt.plot(sec * dt * np.arange(Ttime), s_inf_array, 'm')
    plt.ylabel('s_inf', fontweight='bold')
    plt.title('KATP Channel Activation', fontsize=sizz, fontweight='bold')

    plt.subplot(6, 1, 6)
    plt.plot(sec * dt * np.arange(Ttime), I_KATP_array, 'k')
    plt.xlabel('Time (sec)', fontsize=sizz, fontweight='bold')
    plt.ylabel('I_KATP', fontweight='bold')
    plt.title('KATP Current', fontsize=sizz, fontweight='bold')
...
results = SNcATPapopNM(
    ...
    # scale_I_KATP = 1,
    delay_ms=500,
    stim_duration_ms=1000,
    stim_amp_pA= 0.0, #50,#*1e-6,
    pulse_width_ms=10,
    stim_freq_hz=20
)

scale_I_KATP=np.arange(0.0,0.5,0.1)
# stim_amp_pA = list(range(50,301,50))
sncfrequency_stim = []
for scale in scale_I_KATP:
  results = SNcATPapopNM(dur= dur,
                       gl=gl,
                       mt=mt,
                       Istim= Istim,
                       filename='ATP_',
